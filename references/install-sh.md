...
```sh
sq() { printf "'%s'" "${1//\'/\'\\\'\'}"; } # single-quote for sh: ' becomes '\''
rm -f "$root/bin/<name>"
cat >"$root/bin/<name>" <<EOF
#!/bin/sh
[ -n "\${SOME_VAR:-}" ] || SOME_VAR=$(sq "$value")
export SOME_VAR
exec $(sq "$share_runtime/<name>.sh") "\$@"
EOF
...
The symlink rule assumes the script's data lives in `share/<name>/` next to it, and two shapes break that assumption in opposite directions. A tool that derives its data directory from its own location in a different shape — ddlc-rofi-theme's switch reaches `<own dir>/../share/rofi/themes` — would be sent one directory off by a symlink into `share/<name>/`. A tool that is the whole product, with no payload beside it, has nothing for the symlink to point past: skvpn is that case, its script deriving none of its paths from its own location and `share/skvpn/` holding only the manifest and the `VERSION` copy. Either gets a **copy** in bin instead — which is also what its Nix package installs, and the installer is that package's projection. Say which of the two it is in a comment at the `put`, since the shapes retire on different conditions; the manifest records the copy like anything else
...
2. remove the parent directories left empty, climbing no higher than the prefix's own top-level ones — `prune()` in the template. `bin/`, `share/` and `etc/` belong to the prefix, not to the install: a shell profile may put `~/.local/bin` on PATH only when it exists, so tidying it away changes PATH at the next login. Never `rm -rf` a shared directory like `share/zsh/site-functions`, and never `rmdir -p`, which climbs past the prefix when the prefix itself is empty;
...
- **`--uninstall --component C` takes one component out** and rewrites the manifest with the rest; a full `--uninstall` consumes everything. `--uninstall` refuses only flags that configure an install
