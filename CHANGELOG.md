...
- ci reference: the update-lock-race canon — a push rejected because the weekly bot bump landed mid-work is rebased onto, re-verified on the fresh lock (fmt, flake check, the fast suite), and pushed; never force-pushed over
...
- ci reference + build.yml template: when the preflight demands runtime tools beyond coreutils, the `install.sh works` step runs under `nix develop` — the runner is not a target distribution and a preflight refusing it is working as designed (virtual-media-devices' first push went red exactly this way)
