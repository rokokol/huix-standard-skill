...
- `check-comments.allow` at the root excuses a rejected parse under `tests/fixtures/`, where `must-fail.sh` leaves a quote open on purpose so the lint pipeline has something it must refuse
...
- `templates/completions/install.sh.bash` collects its candidates with a `while IFS= read -r` loop instead of `mapfile`, which bash gained in 4.0, so the file works when a stock macOS bash 3.2 sources it; its header names `check-sh.sh -c` as the check that holds it, as the zsh file's header now does too
- `templates/completions/install.sh.bash` opens with `# shellcheck shell=bash`, the dialect line the bash-best-practices skill asks of a completion file that has no shebang; the template lacked it, so five repositories copied it without
